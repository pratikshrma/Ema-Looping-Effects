...
import PhyllotaxisCamera from './PhyllotaxisCamera'
...
const PhyllotaxisScene = ({ cameraRef }: { cameraRef: React.RefObject<PatternCamera | null> }) => {
  const scene = useThree((s) => s.scene)
  const sheet = useCurrentSheet()

  useEffect(() => {
    if (!sheet) return
    const obj = sheet.object('Phyllotaxis / Background', {
  ...
  return (
    <>
      <PhyllotaxisCamera ref={cameraRef} />
      <e.group theatreKey="Phyllotaxis / Model">
        <Phyllotaxis />
      </e.group>
    </>
  )
