...
import BentSpineCamera from './BentSpineCamera'
...
const BentSpineScene = ({ cameraRef }: { cameraRef: React.RefObject<PatternCamera | null> }) => {
  const scene = useThree((s) => s.scene)
  const sheet = useCurrentSheet()

  useEffect(() => {
    if (!sheet) return
    const obj = sheet.object('BentSpine / Background', {
  ...
  return (
    <>
      <BentSpineCamera ref={cameraRef} />
      <e.group theatreKey="BentSpine / Model">
        <BentSpine />
      </e.group>
    </>
  )
