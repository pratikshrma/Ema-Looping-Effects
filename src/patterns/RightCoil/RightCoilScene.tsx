...
import RightCoil from './RightCoil'
import RightCoilCamera from './RightCoilCamera'
...
const RightCoilScene = ({ cameraRef }: { cameraRef: React.RefObject<PatternCamera | null> }) => {
  const scene = useThree((s) => s.scene)
  const sheet = useCurrentSheet()

  useEffect(() => {
    if (!sheet) return
    const obj = sheet.object('RightCoil / Background', {
  ...
  return (
    <>
      <RightCoilCamera ref={cameraRef} />
      <e.group theatreKey="RightCoil / Model">
        <RightCoil />
      </e.group>
    </>
  )
