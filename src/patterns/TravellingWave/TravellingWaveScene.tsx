...
import TravellingWaveCamera from './TravellingWaveCamera'
...
const TravellingWaveScene = ({ cameraRef }: { cameraRef: React.RefObject<PatternCamera | null> }) => {
  const scene = useThree((s) => s.scene)
  const sheet = useCurrentSheet()

  useEffect(() => {
    if (!sheet) return
    const obj = sheet.object('TravellingWave / Background', {
  ...
  return (
    <>
      <TravellingWaveCamera ref={cameraRef} />
      <e.group theatreKey="TravellingWave / Model">
        <TravellingWave />
      </e.group>
    </>
  )
...
export default TravellingWaveScene
