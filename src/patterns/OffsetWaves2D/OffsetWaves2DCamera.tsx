...
export default function OffsetWaves2DCamera({ ref: cameraRef }: { ref: React.RefObject<PatternCamera | null> }) {
  const sheet = useCurrentSheet()

  useEffect(() => {
    if (!sheet) return
    const obj = sheet.object('OffsetWaves2D / Camera / Zoom', {
  ...
  return (
    <OrthographicCamera
      theatreKey="OffsetWaves2D / Camera / Transformation"
      ref={cameraRef}
      makeDefault={false}
      position={POSITION}
    />
